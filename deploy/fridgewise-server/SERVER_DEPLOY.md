# FridgeWise — Linux server publish guide
...
```bash
scp fridgewise-server.zip user@YOUR_SERVER_IP:/home/user/
```
...
```bash
cd /home/user
unzip fridgewise-server.zip
cd fridgewise-server
```
...
```json
{"status":"ok","service":"fridgewise-api"}
```
...
Then open `https://fridgewise.yourdomain.com` — the app calls `/api` on the same domain automatically.
