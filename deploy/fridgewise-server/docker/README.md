...
| Service | URL |
|---------|-----|
| Web app | http://YOUR_SERVER:8080 |
| API docs | http://YOUR_SERVER:8000/docs |
| API via web proxy | http://YOUR_SERVER:8080/api/health |
...
```bash
# API
docker build -f docker/api/Dockerfile -t fridgewise-api .
...
1. Put **Caddy** or **nginx** on the host in front of port 8080 with HTTPS (Let's Encrypt).
2. Do **not** expose port 8000 publicly if you only need the web UI (remove `ports` for `api` in compose and keep internal network only).
